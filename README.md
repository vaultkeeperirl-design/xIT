...
*   **Dead Air Removal:** Utilizes a single-pass FFmpeg `filter_complex` (trim + concat) to synchronously remove silent segments without generating intermediate files, avoiding audio/video desync and disk I/O bottlenecks. Session-based dead air removal isolates intermediate files to cleanly manage disk usage.
