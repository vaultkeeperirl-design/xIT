...
export function useProject() {
  ...
  const addCaptionClip = useCallback((
    ...
    const safeStart = Number.isFinite(start) ? Math.max(0, start) : 0;
    const safeDuration = Number.isFinite(duration) ? Math.max(0, duration) : 5; // Default to 5s if invalid
  ...
  const addCaptionClipsBatch = useCallback((
    ...
    for (const caption of captions) {
      ...
      const safeDuration = Number.isFinite(caption.duration) ? Math.max(0, caption.duration) : 5;
