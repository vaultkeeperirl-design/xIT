...
export default function Home() {
  ...
  // ⚡ Bolt: Cache active timeline tab in a useMemo to prevent multiple O(N) array lookups
  // This avoids redundant .find() calls during render and callback executions
  const activeTab = useMemo(() => {
    return activeTabId === 'main' ? undefined : timelineTabs.find(tab => tab.id === activeTabId);
            ...
            <div className="flex-1 overflow-hidden relative">
              <div className={`absolute inset-0 ${activeAgent === 'director' ? '' : 'hidden'}`}>
                <AIPromptPanel
                  ...
                  selectedClipId={selectedClipId}
                  activeTabId={activeTabId}
                  editTabAssetId={activeTab?.assetId}
                  editTabClips={activeTab?.clips}
                />
              </div>
